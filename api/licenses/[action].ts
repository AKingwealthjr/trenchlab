...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (action === 'activate') {
      ...
      const key = String(body.licenseKey || ''); if (!key) return sendJson(res, 400, { success: false, error: 'LICENSE_REQUIRED', message: 'Enter your license key.' });
      const db = firebaseAdmin().db; const snapshot = await db.collection('licenses').where('keyHash', '==', hash(key)).limit(1).get();
      if (snapshot.empty) return sendJson(res, 422, { success: false, error: 'INVALID_LICENSE', message: 'This license key could not be verified.' });
      ...
      await db.runTransaction(async tx => {
        ...
        tx.update(ref, { status: 'active', assignedUid: user.uid, assignedEmail: user.email || null, activatedAt: now(), updatedAt: now(), activationCount: (license.activationCount || 0) + (license.assignedUid ? 0 : 1) });
        tx.set(db.collection('users').doc(user.uid), { accessStatus: 'active', licenseId: ref.id, licenseActivatedAt: now(), accessExpiresAt: license.expiresAt || null }, { merge: true });
      });
    ...
    if (action === 'generate') {
      const rawKey = createKey(); const id = db.collection('licenses').doc().id;
      await db.collection('licenses').doc(id).set({ keyHash: hash(rawKey), keyPrefix: rawKey.slice(0, 8), status: 'available', assignedUid: null, assignedEmail: body.assignedEmail || null, createdAt: now(), updatedAt: now(), activatedAt: null, expiresAt: body.expiresAt || null, maxActivations: 1, activationCount: 0, createdBy: admin.uid, notes: String(body.notes || '') });
    ...
    if (['revoke', 'suspend', 'reactivate'].includes(action)) {
      ...
      if (!snap.exists) return sendJson(res, 404, { success: false, error: 'LICENSE_NOT_FOUND', message: 'License not found.' });
    ...
    return sendJson(res, known.includes(code) ? 422 : 500, { success: false, error: known.includes(code) ? code : 'LICENSE_ERROR', message: known.includes(code) ? code.replaceAll('_', ' ') : 'License operation failed.' });
  }
}
